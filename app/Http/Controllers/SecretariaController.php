...
class SecretariaController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            'nombres'=>'required',
            'apellidos'=>'required',
            'run'=>'required|unique:secretarias',
            'fono'=>'required',
            'direccion'=>'required',
        ...
        $secretaria->run = $request->run;
        $secretaria->fono = $request->fono;
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            'nombres'=>'required',
            'apellidos'=>'required',
            'run'=>'required|unique:secretarias,run,'.$secretaria->id,
            'fono'=>'required',
            'direccion'=>'required',
            'fecha_nacimiento'=>'required',
            'email'=>'required|max:200|unique:users,email,'.$secretaria->user->id,
        ...
        return redirect()->route('admin.secretaria.index')
        ->with('mensaje','¡Se ha actualizado a la secreraria de la manera correcta!')
        ->with('icono','success');
    }
    public function confirmDelete($id){
        $secretaria = Secretaria::with('user')->findOrFail($id);
        return view('admin.secretaria.delete',compact('secretaria'));
    }
    /**
    ...
    public function destroy($id)
    {
        ...
        $secretaria->delete();
        return redirect()->route('admin.secretaria.index')
            ->with('mensaje','¡Se ha eliminado a la secretaria de la manera correcta!')
            ->with('icono','success');
